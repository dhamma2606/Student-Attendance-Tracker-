import numpy as np
import pandas as pd
students = pd.read_csv("/content/students (1).csv")["Name"].values
n_students = len(students)
days = 30

prob_present = np.concatenate([
    np.full(n_students // 2, 0.8),
    np.full(n_students - n_students // 2, 0.4)
])

att = np.array([
    np.random.choice(['P', 'A'], p=[p, 1 - p], size=days)
    for p in prob_present
])
...
df.to_csv("students_attendance.csv", index=True)
daily_info.to_csv("daily_attendance_summary.csv", index=True)
